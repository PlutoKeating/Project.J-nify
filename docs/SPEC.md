...
1. 信用卡账单：录入当天只确认；到期前十天给一次顺手窗口；用户说等工资则改期，不每日追问。
2. 晒被子：无死线，绑定连续晴天微风窗口；用户选再等等后，不每周追问。
3. 回小明：不设下午三点闹钟；在用户刷手机超过阈值或周末上午出现，并附 30 秒草稿。
4. 小组作业：Day1 拆块；Day5 只搜资料；Day10 给每天 40 分钟方案；Day13 给“今晚三小时”或“延期申请”。
5. 七天退货：第 3 天顺路提醒，第 5 天提供上门取件；允许“不换了”作为体面结局。
...
M1 信号	4 周	日历/天气/粗粒度位置/通知交互；频控红线	提醒均有理由；打扰预算生效	
