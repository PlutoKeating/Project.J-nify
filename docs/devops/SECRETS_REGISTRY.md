...
| --- | --- | --- | --- |
| `SMTP_HOST` = smtp.yeah.net | 生产发信（Supabase 自定义 SMTP）+ Worker 告警邮件 | GitHub Actions Secrets（已存）+ CF Worker Secrets `SMTP_HOST`（已同步 2026-08-29）| `sync-worker-secrets` 工作流 / `gh secret set` |
...
| `ADMIN_USERNAME` / `ADMIN_PASSWORD` | admin 面板登录账号口令 | 待用户创建 → GitHub Actions Secrets → 同步 CF Worker Secrets | `gh secret set` 后运行 `sync-worker-secrets` |
...
> **凭据同步机制（2026-08-29 新增）**：`.github/workflows/sync-worker-secrets.yml`（特性分支 `chore/sync-worker-secrets`，待批准合入 main）手动触发，将 GH Actions Secrets 中登记的值同步为 CF Worker Secrets（`wrangler secret put` + `secret list` 验证），真值不落库。
